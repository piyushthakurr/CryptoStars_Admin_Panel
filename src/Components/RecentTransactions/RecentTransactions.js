...
function RecentTransactions({ handleCloseRecent }) {
    ...
    let nwRecentTransaction ;
    if (nwChangeId== 338){
        nwRecentTransaction= URL.CRONOS_EXPLORER;
    }else if (nwChangeId== 97){
        nwRecentTransaction= URL.BNB_EXPLORER;
    }else if (nwChangeId== 43113){
        nwRecentTransaction= URL.AVA_EXPLORER;
    }
                        ...
                        {recentTransactions.length > 0 ?
                            <ul>
                                {recentTransactions.map((t, index) =>
                                    <li key={index}>
                                        <a target="_blank" rel="noreferrer noopener" href={`${nwRecentTransaction}/tx/${t.tx}`} className="success">{t.message}
